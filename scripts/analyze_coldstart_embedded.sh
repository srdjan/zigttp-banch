#!/bin/bash
# Cold start analysis for embedded bytecode build (no handler file argument)
...
for warmup in {1..3}; do
    ...
    "$ZIGTTP_BIN" -p "$PORT" -q >/dev/null 2>&1 &
    server_pid=$!
    while ! lsof -ti:$PORT >/dev/null 2>&1; do
...
for i in $(seq 1 $iterations); do
    ...
    # Start server WITHOUT handler file argument (uses embedded bytecode)
    "$ZIGTTP_BIN" -p "$PORT" -q >/dev/null 2>&1 &
    server_pid=$!
